  ...
  setup(props, { slots, emit }) {
    ...
    return () => {
      ...
      if (props.boundaryLinks) {
        ...
        const firstPageSlot = slots.firstPageIcon?.({ disabled }) ?? firstPageIcon
      ...
      if (props.directionLinks) {
        ...
        const disabled = props.currentPage === 1
        const previousSlot = slots.previousIcon?.({ disabled }) ?? previousIcon
      ...
      if (props.directionLinks) {
        ...
        const nextSlot = slots.nextIcon?.({ disabled }) ?? nextIcon
      ...
      if (props.boundaryLinks) {
        ...
        const disabled = props.currentPage === props.totalPages
        const firstPageSlot = slots.lastPageIcon?.({ disabled }) ?? lastPageIcon
